...
8. Perform operations by a grouping variable
...
See `?nasa` for description of the NASA spatio-temporal data, we will
make a copy of it:

```{r}
# nasa is included in `dplyr` - converting to a data.frame
ex_data = as.data.frame(nasa)
```
